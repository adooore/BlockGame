...
const PersistedStore = {
    ...
    setWebSocket(ws) {
        this._ws = ws;
        console.log('[PersistedStore] WebSocket 已设置');
    },
    ...
    /**
     * 将 _cache 中的 gameSettings / playerColors 同步到各模块的 _memCache。
     * 必须与 load()、initFromServer() 保持一致，否则 _getSync 会优先返回陈旧 _memCache，
     * 出现「设置界面显示关、ControllerManager 仍按默认开」等问题。
     */
    _syncModuleCachesFromMainCache() {
        if (!this._cache) this._cache = {};
        if (this._cache.gameSettings) {
            this.gameSettings._memCache = { ...this._cache.gameSettings };
        } else {
            this.gameSettings._memCache = null;
        }
        if (this._cache.playerColors) {
            this.playerColors._memCache = { ...this._cache.playerColors };
        } else {
            this.playerColors._memCache = null;
        }
    },

    // 从服务器连接消息初始化数据（字符串为 JSON 存档；对象则直接使用）
    initFromServer(gameData) {
        if (gameData == null || gameData === '') return;
        try {
            ...
            this._syncModuleCachesFromMainCache();
            console.log('[PersistedStore] 从服务器初始化成功:', this._cache);
        } catch (e) {
            console.warn('[PersistedStore] 解析服务器数据失败:', e);
            this._cache = {};
            this._cacheLoaded = true;
            this._syncModuleCachesFromMainCache();
        }
    ...
    async load() {
        ...
        try {
            console.log('[PersistedStore] 环境检测: _isDesktopApp =', this._isDesktopApp);

            if (this._isDesktopApp) {
                // 桌面应用：数据已通过 WebSocket connected 消息初始化
                // 如果还没初始化，等待一下
                if (!this._cache) {
                    console.log('[PersistedStore] 等待服务器数据...');
                    this._cache = {};
                }
                console.log('[PersistedStore] 使用服务器文件存储');
            } else {
                // 纯浏览器环境：使用 localStorage
                console.log('[PersistedStore] 使用 localStorage 存储');
                const saved = localStorage.getItem('blockgame_data');
                this._cache = saved ? JSON.parse(saved) : {};
                console.log('[PersistedStore] 从 localStorage 加载');
            }
        } catch (e) {
            console.warn('[PersistedStore] 加载失败，使用默认值:', e);
            this._cache = {};
        }
        ...
        console.log('[PersistedStore] 加载完成', this._cache);
        return this._cache;
    },
    ...
    async save() {
        if (!this._cache) {
            console.warn('[PersistedStore] _cache 为空，跳过保存');
            return;
        }
        ...
        try {
            ...
            if (this._isDesktopApp && this._ws && this._ws.readyState === WebSocket.OPEN) {
                ...
                console.warn('[PersistedStore] ✗ WebSocket 未连接，无法保存');
                console.warn('[PersistedStore] 尝试使用 localStorage 作为备份...');
                localStorage.setItem('blockgame_data', jsonStr);
            }
            console.log('[PersistedStore] 保存完成', this._cache);
        } catch (e) {
            console.error('[PersistedStore] 保存失败:', e);
        }
    },
    ...
    records: {
        ...
        async getBestTime(levelId = 'eatAndAvoid') {
            const data = await PersistedStore.load();
        ...
        async setBestTime(levelId, time) {
            ...
            if (current === 0 || time < current) {
                ...
                console.log(`[PersistedStore] 新纪录！关卡=${levelId}, 时间=${time}秒`);
                return true;
            }
            return false;
        },
    ...
    gameSettings: {
        ...
        _getSync() {
            ...
            if (PersistedStore._cache && PersistedStore._cache.gameSettings) {
                this._memCache = PersistedStore._cache.gameSettings;
        ...
        _setSync(newData) {
            ...
            if (PersistedStore._cache) {
                PersistedStore._cache.gameSettings = this._memCache;
            }
            // 延迟保存，避免频繁写入
            this._scheduleSave();
        },
        ...
        getAll() {
            return {
                ...
                showPlayerNumber: this.getShowPlayerNumber(),
        ...
        /**
         * 获取是否显示玩家序号（头顶 P1/P2/P3）
         * @returns {boolean} 默认 true，局内局外统一
         */
        getShowPlayerNumber() {
            const val = this._getSync().showPlayerNumber;
            return val !== false;
        },
         ...
         * @param {boolean} enabled
         */
        setShowPlayerNumber(enabled) {
            this._setSync({ showPlayerNumber: !!enabled });
        },
    ...
    playerColors: {
        ...
        getSavedColors() {
            ...
            if (this._memCache) return this._memCache;
            // 从 _cache 读取
            if (PersistedStore._cache && PersistedStore._cache.playerColors) {
                this._memCache = PersistedStore._cache.playerColors;
                return this._memCache;
            }
            return {};
        },
        ...
        saveColors(colors) {
            ...
            if (PersistedStore._cache) {
                PersistedStore._cache.playerColors = colors;
            }
            // 延迟保存
            this._scheduleSave();
        },
    ...
    debug: {
        ...
        async importData(jsonStr) {
            try {
                PersistedStore._cache = JSON.parse(jsonStr);
                await PersistedStore.save();
                return true;
            } catch (e) {
                console.error('[PersistedStore] 导入失败:', e);
                return false;
            }
        },
         ...
         */
        async clearAll() {
            PersistedStore._cache = {};
            await PersistedStore.save();
            console.log('[PersistedStore] 已清空所有数据');
        }
    },
...
if (typeof window !== 'undefined') {
    window.PersistedStore = PersistedStore;
    window.addEventListener('DOMContentLoaded', () => {
        PersistedStore.load().then(() => {
            console.log('[PersistedStore] 初始化完成');
        });
    });

    // 切页前冲刷延迟保存，避免刚修改设置就跳页导致数据丢失
    window.addEventListener('pagehide', () => {
        PersistedStore.flushPendingSaves();
    });
